...
| `lfrGitUpdateMaster` | `lfrgum` | Update each mirror configured in `LFR_GIT_MASTER_MIRRORS` from its `<remote>/master` (e.g. `master` from upstream, `masterBrian` from brian) and sync the team fork (`lfrGitSync`/`lfrGitSyncEE`); run it from any worktree, a mirror checked out in another one is fast-forwarded inside it; `-r` rebases the current branch onto a target (default `upstream/master`, or pass a remote/branch), `-f` forces it, `-p` force-pushes it. Takes `[-r] [-f] [-p] [rebase-target]` | `LfrGit/` |
