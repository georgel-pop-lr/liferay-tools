...
| `lfrGitUpdateMaster` | `lfrgum` | Update your fork's master from a remote (default `upstream`), keep your local master current without checking it out, sync the team fork (`lfrGitSync`/`lfrGitSyncEE`); `-r` rebases your current branch onto the fresh master (only when it moved), `-f` forces that rebase, `-p` then force-pushes it. Takes `[-r] [-f] [-p] [remote]` | `LfrGit/` |
