...
| `lfrGitUpdateMaster` | `lfrgum` | Update your local master branch from a remote (default `upstream`), push it to your fork, sync the team fork (`lfrGitSync`/`lfrGitSyncEE`); `-r` rebases your current branch onto it (only when master moved), `-f` forces that rebase even when it did not, `-p` then force-pushes it. Takes `[-r] [-f] [-p] [remote] [local-branch]` | `LfrGit/` |
