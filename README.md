...
| `lfrPulls` | `lfrp` | The three queues a pull travels through, each with a `STATUS` (conflict, changes needed, in review, ready): yours on the Brian mirror with its `AHEAD` merge-queue position, your team's fork, and your own fork where teammates await your review. Name a team or a user for theirs (`lfrPulls frontend`, `lfrpf`), `lfrPulls teams` (`lfrpteams`) for the list. Also `lfrPulls <TICKET>` / `lfrPulls ticket` (`lfrpt`) for every pull ever opened for one ticket plus what it landed on the master ref, `lfrPulls week` (`lfrpw`) for your pulls closed in the last days with merged/rejected status, and `lfrPulls stats` (`lfrps`) for per-month counts plus the same three queues in full detail. | `LfrPulls/` |
