...
| `lfrWorktree` | `lfrw` | Create a git worktree off a base ref, wired to its own bundle and database (per-user props copied and repointed, bundle dir created with `portal-ext.properties` on `portal-<branch>`). Asks before reusing a bundle dir already there, and offers to run `lfrWorktreeIdeaInit` at the end (`LFR_WORKTREE_IDEA=1`/`0` answers that in advance) | `LfrRepo/` |
| `lfrWorktreeRename` | `lfrwn` | Rename a worktree to another branch instead of removing it and building a fresh one: the branch, the directory (`liferay-portal-<new>`), the bundle directory (`liferay-bundle-<new>`), the bundle path in the per-user props, the database (`portal-<new>`, renamed with its data in it), and IntelliJ's project. Shows the four moves and asks once; refuses while that bundle's Tomcat runs, or when another branch, the new directory or bundle directory exists. The old name is read off the worktree directory, so giving it the name the branch already has brings a worktree whose branch was renamed by hand back into line. `osgi/state` is deleted, being the one part of a built bundle that names its own absolute path. Leaves a bundle that is not this worktree's own alone. Takes `[--keep-database]` | `LfrRepo/` |
...
| `lfrWorktreeIdeaClean` / `lfrWorktreeIdeaCleanDry` | | Make IntelliJ forget every `liferay-portal-<branch>` project whose directory is gone, and delete its caches: the leftovers of a worktree removed by hand or while an IDE was open. Offers to close IntelliJ first when it is running | `LfrRepo/` |
| `lfrWorktreeIdeaInit` | | Give a worktree the IntelliJ project a clone already has: the project model, every `.iml`, and the run configurations (the Debugg profiles) written to `.idea/runConfigurations`. Leaves out the data sources, the shelf, and the Tomcat configurations, which are bound to the source clone's bundle. Ends by putting the worktree in IntelliJ's recent projects, which `File > Open` cannot reach until the IDE restarts, unless IntelliJ is running, in which case it prints the launcher line instead. Takes `[--redo|--recent]` | `LfrRepo/` |
...
| `lfrBundle` | `lfrb` | Toggle Liferay bundles: shows each bundle's state and starts the one you pick/name if stopped (forwarding start-flags like `-c`) or stops it if running. Also `lfrBundle status` (ports, the deploying checkouts, and the flags each bundle was launched with), `lfrBundle stop-all`, `lfrBundle cd` (jump to a bundle's Liferay home without starting it), and `lfrBundle upgrade` (run a stopped bundle's database upgrade tool). `lfrRunBundle` / `lfrrb` alias it. | `LfrBundle/` |
...
| `lfrPulls` | `lfrp` | The four queues a pull travels through, each with a `STATUS` (conflict, changes needed, in review, ready) and an `ON YOU` saying whether the next move is yours: yours on the Brian mirror with its `AHEAD` merge-queue position, your team's fork (kept to your own pulls, the ones on you, and the untriaged ones), your own fork where teammates await your review, and your backports on the EE repo (`lfrPulls ee`, `lfrpe`). Name a team or a user for theirs (`lfrPulls frontend`, `lfrpf`), or an `owner/repo` for another repo (`lfrPulls liferay/liferay-portal-ee`), `lfrPulls teams` (`lfrpteams`) for the list. Anywhere `mine` is taken a login works instead, so `lfrPulls stats nikki-pru` asks the whole question about somebody else. Also `lfrPulls <TICKET>` / `lfrPulls ticket` (`lfrpt`) for every pull ever opened for one ticket plus what it landed on the master ref, `lfrPulls week` (`lfrpw`) for pulls closed in the last days with merged/rejected status, and `lfrPulls stats` (`lfrps`) for per-month counts plus the same queues in full detail. | `LfrPulls/` |
